...
class XRayConfig(dict):
    ...
    @staticmethod
    def _get_inbound_accounts(inbound: dict) -> list:
        if not inbound.get("settings"):
            inbound["settings"] = {}

        settings = inbound["settings"]

        if inbound["protocol"] == ProxyTypes.MTProto.value:
            for key in ("users", "user", "clients"):
                if key in settings:
        ...
        if "clients" not in settings or settings["clients"] is None:
            settings["clients"] = []
    ...
    def _resolve_inbounds(self):
        for inbound in self['inbounds']:
            ...
            self._get_inbound_accounts(inbound)
        ...
        with GetDB() as db:
            ...
            for proxy_type, rows in grouped_data.items():
                ...
                for inbound in inbounds:
                    accounts = self._get_inbound_accounts(config.get_inbound(inbound['tag']))
